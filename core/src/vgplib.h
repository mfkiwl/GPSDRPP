...
#define VGP_VERSION         "2.0.1"

#define GPIO_SWPORTA_DR     0x0000u
#define GPIO_SWPORTA_DDR    0x0004u
#define GPIO_EXT_PORTA      0x0050u

#define PMUGRF              0xff320000u
#define GRF                 0xff770000u

#define GPIO_INPUT    0
#define GPIO_OUTPUT   1

static const uint32_t GPIO_BASE[] = { 0xff720000u, 0xff730000u, 0xff780000u, 0xff788000u, 0xff790000u };

static const int32_t GPIO_IOMUX[5][4] = {
...
static const char * GPIO_DIRECTION[] __attribute__((unused)) = { "IN", "OUT" };
...
// Optional libgpiod caching (for GUI / high-frequency polling)
// When enabled, gpiod chips/lines stay open and requested, making get()/set() much faster.
...
static const char * FUNCTIONS[][4] __attribute__((unused)) = { {NULL, NULL, NULL, NULL},
  /*3.3V*/{"","", "", ""},                            /*5V*/  {"", "", "", ""},
...
typedef struct {
  ...
  int fd;            // local gpiod event fd (when monitoring via libgpiod in-process)
  int active;        // 1 if monitoring is active (either local gpiod or vgpd SUBSCRIBE)
  int closing;       // 1 if requested to stop
  time_t start_at;   // when to start monitoring (delay support)
} MonitorThread;
